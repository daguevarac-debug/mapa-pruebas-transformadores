...
export default function HomePage() {
  ...
  const endPan = (event: PointerEvent<SVGSVGElement>) => {
    if (panRef.current.pointerId === event.pointerId) { event.currentTarget.releasePointerCapture(event.pointerId); setIsDragging(false); }
  };
  ...
  return (
    <main className="study-shell mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-8 md:px-8">
      ...
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white/85 p-4 shadow-sm backdrop-blur" aria-label="Búsqueda y filtros">
        <div className="flex flex-wrap items-end justify-between gap-3"><div className="min-w-[220px] flex-1"><label htmlFor="procedure-search" className="text-sm font-semibold text-slate-800">Buscar una prueba</label><input id="procedure-search" value={query} onChange={(event) => { setQuery(event.target.value); selectProcedure(null); }} placeholder="Nombre, T50 o palabra de la ficha" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none ring-sky-300 focus:ring-2" /></div><div className="text-sm text-slate-700" aria-live="polite"><strong>{filteredProcedures.length}</strong> de {procedures.length} pruebas coinciden{activeFilterCount ? ` con ${activeFilterCount} filtro${activeFilterCount === 1 ? "" : "s"} activo${activeFilterCount === 1 ? "" : "s"}` : ""}.</div>{activeFilterCount > 0 && <button type="button" onClick={clearFilters} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-[background-color,transform] duration-100 ease-out hover:bg-slate-50 active:scale-[0.97]">Limpiar filtros</button>}</div>
        <div className="mt-4 border-t border-slate-200 pt-3"><p className="text-sm font-semibold text-slate-800">Familia técnica</p><div className="mt-2 flex flex-wrap gap-2">{technicalFamilies.map((family) => <button key={family} type="button" onClick={() => toggleFamily(family)} aria-pressed={families.includes(family)} className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-[border-color,background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${families.includes(family) ? "border-teal-600 bg-teal-700 text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}>{family}</button>)}</div></div>
        <div className="mt-4 border-t border-slate-200 pt-3"><p className="text-sm font-semibold text-slate-800">Contexto de ejecución documentado</p><div className="mt-2 flex flex-wrap gap-2">{documentedContexts.map((context) => <button key={context.id} type="button" onClick={() => toggleContext(context.id)} aria-pressed={contexts.includes(context.id)} className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-[border-color,background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${contexts.includes(context.id) ? "border-violet-600 bg-violet-700 text-white" : "border-violet-200 bg-violet-50 text-violet-900 hover:bg-violet-100"}`}>{context.label}</button>)}</div></div>
      </section>

      <section className="mt-4 rounded-2xl border border-slate-200 bg-white/85 p-2 shadow-sm backdrop-blur"><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setViewMode("explore")} className={`rounded-xl px-4 py-3 text-sm font-semibold transition-[background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${viewMode === "explore" ? "bg-sky-100 text-sky-900 shadow-inner" : "bg-white text-slate-700 hover:bg-slate-50"}`}>Explorar pruebas</button><button type="button" onClick={() => setViewMode("map")} className={`rounded-xl px-4 py-3 text-sm font-semibold transition-[background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${viewMode === "map" ? "bg-sky-100 text-sky-900 shadow-inner" : "bg-white text-slate-700 hover:bg-slate-50"}`}>Mapa de relaciones</button></div></section>

      {viewMode === "explore" && <section className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_1fr]"><div className="rounded-2xl border border-slate-200 bg-white/85 p-4 shadow-sm backdrop-blur"><h2 className="px-2 text-xl font-semibold text-[#0d2b45]">Catálogo de procedimientos</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{filteredProcedures.map((procedure) => <button key={procedure.code} type="button" onClick={() => selectProcedure(procedure.code)} className={`rounded-xl border px-4 py-4 text-left transition-[border-color,background-color,box-shadow,transform] duration-150 ease-out active:scale-[0.97] ${selectedCode === procedure.code ? "border-sky-300 bg-sky-50 shadow" : "border-slate-200 bg-white hover:border-sky-200 hover:bg-slate-50"}`}><p className="text-xs font-semibold uppercase tracking-[0.08em] text-teal-700">{procedure.code}</p><h3 className="mt-1 text-base font-semibold leading-tight text-slate-900">{procedure.name}</h3><span className={`mt-2 inline-flex rounded-full border px-2 py-1 text-xs font-medium ${statusClass(procedure.studyStatus)}`}>{statusLabel(procedure.studyStatus)}</span><p className="mt-2 text-sm text-slate-600">{procedureFamilies[procedure.code]}</p></button>)}</div>{filteredProcedures.length === 0 && <p className="m-2 rounded-lg border border-dashed border-slate-300 px-3 py-4 text-sm text-slate-600">No hay pruebas que coincidan con los filtros actuales.</p>}</div><ProcedurePanel key={selectedCode ?? "empty"} procedure={selectedProcedure} relations={selectedRelations} onInspectRelation={setInspectedRelationKey} activeGlossaryTerm={activeGlossaryTerm} onSelectGlossaryTerm={setActiveGlossaryTerm} /></section>}

      {viewMode === "map" && <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,360px)]"><div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm backdrop-blur"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold text-[#0d2b45]">Mapa de relaciones</h2>{activeFilterCount > 0 && <p className="mt-1 inline-flex items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-2.5 py-1 text-xs font-semibold text-teal-900"><span className="h-1.5 w-1.5 rounded-full bg-teal-600" /> Encuadre filtrado: {activeMapLabel}</p>}</div><div className="flex flex-wrap items-center gap-2 text-xs"><div className="inline-flex overflow-hidden rounded-full border border-slate-300 bg-white shadow-sm" aria-label="Controles del mapa"><button type="button" onClick={zoomOut} disabled={mapZoom <= 0.6} className="px-3 py-1.5 font-semibold text-slate-700 transition-[background-color,transform] duration-100 ease-out hover:bg-slate-50 active:scale-[0.97] disabled:cursor-not-allowed disabled:text-slate-300" aria-label="Alejar mapa">−</button><button type="button" onClick={fitMap} className="min-w-14 border-x border-slate-200 px-2 py-1.5 font-medium text-slate-600 transition-[background-color,transform] duration-100 ease-out hover:bg-slate-50 active:scale-[0.97]" aria-label={activeFilterCount ? "Encuadrar resultados filtrados" : "Encuadrar todo el mapa"}>{Math.round(mapZoom * 100)}%</button><button type="button" onClick={zoomIn} disabled={mapZoom >= 1.6} className="px-3 py-1.5 font-semibold text-slate-700 transition-[background-color,transform] duration-100 ease-out hover:bg-slate-50 active:scale-[0.97] disabled:cursor-not-allowed disabled:text-slate-300" aria-label="Acercar mapa">+</button></div><button type="button" onClick={() => setShowDirectional((value) => !value)} aria-pressed={showDirectional} className={`rounded-full border px-3 py-1.5 font-medium transition-[border-color,background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${showDirectional ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-slate-300 bg-white text-slate-500"}`}>Relaciones direccionales {showDirectional ? "visibles" : "ocultas"}</button><button type="button" onClick={() => setShowComplementary((value) => !value)} aria-pressed={showComplementary} className={`rounded-full border px-3 py-1.5 font-medium transition-[border-color,background-color,color,transform] duration-150 ease-out active:scale-[0.97] ${showComplementary ? "border-violet-300 bg-violet-50 text-violet-800" : "border-slate-300 bg-white text-slate-500"}`}>Montaje compartido {showComplementary ? "visible" : "oculto"}</button></div></div>
...
function RelationPath({ relation, relationIndex, relationCount, selectedCode, inspected, onInspect }: { relation: ProcedureRelation; relationIndex: number; relationCount: number; selectedCode: string | null; inspected: boolean; onInspect: (key: string) => void }) {
  ...
  return <path d={`M ${startX} ${startY} Q ${controlX} ${controlY} ${endX} ${endY}`} fill="none" stroke={color} strokeWidth={inspected ? "7" : "3.5"} strokeOpacity={inspected ? "0.85" : "1"} strokeDasharray={shared ? "4 7" : undefined} strokeLinecap="round" markerEnd={shared ? undefined : outgoing ? "url(#arrow-green)" : "url(#arrow-orange)"} style={{ transition: "stroke-width 120ms ease-out" }} tabIndex={0} role="button" aria-label={`Ver trazabilidad de la relación ${relation.from} ${shared ? "con" : "hacia"} ${relation.to}`} onClick={(event) => { event.stopPropagation(); onInspect(relationKey(relation)); }} onMouseEnter={() => onInspect(relationKey(relation))} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onInspect(relationKey(relation)); } }} />;
}

function RelationTrace({ relation, onClose }: { relation: ProcedureRelation; onClose: () => void }) {
  const shared = relation.type === "shared_setup";
  return <section className="slide-in mt-4 rounded-xl border border-sky-200 bg-sky-50/80 p-4" aria-live="polite"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.1em] text-sky-800">Trazabilidad de la relación</p><h3 className="mt-1 text-base font-semibold text-slate-900">{shared ? `${procedureName(relation.from)} ↔ ${procedureName(relation.to)}` : `${procedureName(relation.from)} → ${procedureName(relation.to)}`}</h3></div><button type="button" onClick={onClose} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-slate-500 transition-[background-color,transform] duration-100 ease-out hover:bg-sky-100 active:scale-[0.95]" aria-label="Cerrar trazabilidad">✕</button></div><div className="mt-3 grid gap-2 text-sm text-slate-700"><p><strong>Tipo:</strong> {typeLabel(relation.type)}.</p><p><strong>Qué la justifica:</strong> {relation.rationale}</p><p><strong>Condición:</strong> {relation.condition}</p>{relation.transferredData && <p><strong>Dato asociado:</strong> {relation.transferredData}</p>}</div></section>;
}
...
function GlossaryText({ children, onSelect }: { children: string; onSelect: (term: GlossaryTerm) => void }) {
  ...
  return <>{pieces.map((piece, index) => {
    ...
    return <button key={`${match.term.id}-${index}`} type="button" onClick={() => onSelect(match.term)} className="rounded border-b border-dashed border-sky-500 px-0.5 font-semibold text-sky-900 decoration-sky-500 underline-offset-2 transition-[background-color] duration-100 hover:bg-sky-100 focus:outline-none focus:ring-2 focus:ring-sky-400" aria-label={`Abrir glosario: ${match.term.label}`}>{piece}</button>;
  })}</>;
}

function GlossaryCard({ term, onClose }: { term: GlossaryTerm; onClose: () => void }) {
  return <section className="slide-in mt-4 rounded-xl border border-indigo-200 bg-indigo-50/80 p-4 shadow-sm" aria-live="polite"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.1em] text-indigo-800">Glosario contextual</p><h3 className="mt-1 text-lg font-semibold text-slate-900">{term.label}</h3></div><button type="button" onClick={onClose} className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-slate-500 transition-[background-color,transform] duration-100 ease-out hover:bg-indigo-100 active:scale-[0.95]" aria-label="Cerrar glosario">✕</button></div><p className="mt-3 text-sm leading-6 text-slate-800">{term.definition}</p><p className="mt-3 rounded-lg border border-indigo-100 bg-white/80 p-3 text-sm leading-6 text-slate-700"><strong>Analogía:</strong> {term.analogy}</p></section>;
}
