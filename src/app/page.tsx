...
export default function HomePage() {
  ...
  const visibleRelations = useMemo(() => selectedRelations.filter((relation) => relation.type === "shared_setup" ? showComplementary : showDirectional), [selectedRelations, showComplementary, showDirectional]);
  ...
  const selectProcedure = (code: string | null) => { setSelectedCode(code); setInspectedRelationKey(null); };
  ...
  const endPan = (event: PointerEvent<HTMLDivElement>) => { if (panRef.current.pointerId === event.pointerId) event.currentTarget.releasePointerCapture(event.pointerId); };
  ...
  return (
    <main className="study-shell mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-8 md:px-8">
      ...
      {viewMode === "explore" && <section className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_1fr]"><div className="rounded-2xl border border-slate-200 bg-white/85 p-4 shadow-sm backdrop-blur"><h2 className="px-2 text-xl font-semibold text-[#0d2b45]">Catálogo de procedimientos</h2><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{filteredProcedures.map((procedure) => <button key={procedure.code} type="button" onClick={() => selectProcedure(procedure.code)} className={`rounded-xl border px-4 py-4 text-left transition ${selectedCode === procedure.code ? "border-sky-300 bg-sky-50 shadow" : "border-slate-200 bg-white hover:border-sky-200 hover:bg-slate-50"}`}><p className="text-xs font-semibold uppercase tracking-[0.08em] text-teal-700">{procedure.code}</p><h3 className="mt-1 text-base font-semibold leading-tight text-slate-900">{procedure.name}</h3><span className={`mt-2 inline-flex rounded-full border px-2 py-1 text-xs font-medium ${statusClass(procedure.studyStatus)}`}>{statusLabel(procedure.studyStatus)}</span><p className="mt-2 text-sm text-slate-600">{procedureFamilies[procedure.code]}</p></button>)}</div>{filteredProcedures.length === 0 && <p className="m-2 rounded-lg border border-dashed border-slate-300 px-3 py-4 text-sm text-slate-600">No hay pruebas que coincidan con los filtros actuales.</p>}</div><ProcedurePanel procedure={selectedProcedure} relations={selectedRelations} onInspectRelation={setInspectedRelationKey} /></section>}

      {viewMode === "map" && <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,360px)]"><div className="rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm backdrop-blur"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-semibold text-[#0d2b45]">Mapa de relaciones</h2><div className="flex flex-wrap items-center gap-2 text-xs"><div className="inline-flex overflow-hidden rounded-full border border-slate-300 bg-white shadow-sm" aria-label="Controles del mapa"><button type="button" onClick={zoomOut} disabled={mapZoom <= 0.6} className="px-3 py-1.5 font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300" aria-label="Alejar mapa">−</button><button type="button" onClick={fitMap} className="min-w-14 border-x border-slate-200 px-2 py-1.5 font-medium text-slate-600 hover:bg-slate-50" aria-label="Encuadrar todo el mapa">{Math.round(mapZoom * 100)}%</button><button type="button" onClick={zoomIn} disabled={mapZoom >= 1.6} className="px-3 py-1.5 font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300" aria-label="Acercar mapa">+</button></div><button type="button" onClick={() => setShowDirectional((value) => !value)} aria-pressed={showDirectional} className={`rounded-full border px-3 py-1.5 font-medium ${showDirectional ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-slate-300 bg-white text-slate-500"}`}>Relaciones direccionales {showDirectional ? "visibles" : "ocultas"}</button><button type="button" onClick={() => setShowComplementary((value) => !value)} aria-pressed={showComplementary} className={`rounded-full border px-3 py-1.5 font-medium ${showComplementary ? "border-violet-300 bg-violet-50 text-violet-800" : "border-slate-300 bg-white text-slate-500"}`}>Montaje compartido {showComplementary ? "visible" : "oculto"}</button></div></div>
        <p className="mt-3 text-sm text-slate-600">Selecciona una prueba para revelar solo sus relaciones. Arrastra el lienzo para recorrerlo y usa el porcentaje para encuadrarlo de nuevo. Las salientes son verdes; las entrantes, naranjas; y las relaciones de montaje compartido, moradas punteadas y sin flecha.</p>
...
function ProcedurePanel({ procedure, relations, onInspectRelation }: { procedure: Procedure | null; relations: ProcedureRelation[]; onInspectRelation: (key: string) => void }) {
